...
Edit the `SimConnect.xml` file located in `%appdata%/Microsoft Flight Simulator` with a text editor of your choice.
Then add a new `<SimConnect.Comm>{:xml}` element as shown below,
replacing `PORT` in the highlighted line with the one that was set in Newsky:

```xml {6} filename="SimConnect.xml" copy
<SimConnect.Comm>
